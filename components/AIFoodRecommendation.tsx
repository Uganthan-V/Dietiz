...
const AIFoodRecommendation: React.FC<AIFoodRecommendationProps> = ({
  ...
  const [showPersonalInfoModal, setShowPersonalInfoModal] = useState(false);
  const [recommendations, setRecommendations] = useState<FoodItem[]>([]);
  const [previousMeals, setPreviousMeals] = useState<FoodItem[]>([]);
  const [isLoading, setIsLoading] = useState(false);
  const [personalInfo, setPersonalInfo] = useState<PersonalInfo | null>(null);
  const [hasApiKey, setHasApiKey] = useState(false);
  const [hasCompletedSetup, setHasCompletedSetup] = useState(false);
  ...
  const initializeData = useCallback(async () => {
    ...
    try {
      ...
      const previous = await StorageService.getPreviousMealsByCategory(selectedMealType);
      setPreviousMeals(previous);
    } catch (error) {
      console.error("Error initializing data:", error);
    } finally {
      setIsLoading(false);
