...
// ✅ FIXED: onComplete is now async
interface PersonalInfoModalProps {
  visible: boolean;
  onClose: () => void;
  onComplete: (info: PersonalInfo) => Promise<void>;
}
...
const PersonalInfoModal: React.FC<PersonalInfoModalProps> = ({
  ...
  const handleComplete = async () => {
    ...
    // ✅ Now safe to await because onComplete is async
    await onComplete(finalPersonalInfo);
  };
  ...
  const calculateTargetCalories = (): number => {
    ...
    let tdee = bmr * (activityMultipliers[personalInfo.activityLevel as keyof typeof activityMultipliers] || 1.2);
  ...
  const renderStep = () => {
    // ... (all your existing renderStep code remains 100% unchanged)
    switch (currentStep) {
      case 0:
        return (
          <View style={styles.stepContainer}>
