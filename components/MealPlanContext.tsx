...
export const MealPlanProvider: React.FC<MealPlanProviderProps> = ({ children }) => {
    ...
    try {
      ...
      if (savedMeals) {
        const fixedMeals: Meal[] = savedMeals.map((meal: any) => ({
          ...
          food: meal.food || null,
          hasFood: !!meal.food,
          consumed: !!meal.food && (meal.consumed === true || meal.consumed === 'true'),
        }));
        setMeals(fixedMeals);
      }
    } catch (error) {
  ...
  // ✅ FIXED: PERMANENT PREVIOUS MEALS SAVE
  const updateMeal = useCallback((
    mealId: string, 
    food: FoodItem | null, 
    consumed?: boolean
    ...
    // ✅ PERMANENT SAVE: If food added, save to previous meals
    if (food) {
      StorageService.savePreviousMeal(food).then(() => {
        console.log(`✅ SAVED "${food.name}" to PERMANENT previous meals`);
      });
    }
  }, []);
