...
class GeminiService {
  ...
  private model: any = null;
  private apiKey: string | null = null;
  private readonly DEFAULT_MODEL = 'gemini-2.5-flash'; // Corrected to a supported model (assuming typo in original)
  ...
  async generateMealRecommendations(
    ...
    try {
      ...
      const prompt = this.buildMealRecommendationPrompt(personalInfo, mealType, currentMeals);
      const result = await this.model.generateContent(prompt);
      const response = await result.response;
      const text = response.text();
      return this.parseAIResponse(text, mealType);
  ...
  private buildMealRecommendationPrompt(
    ...
    return `You are an expert dietitian specializing in personalized meal recommendations. Use DYNAMIC PERCENTAGE ALLOCATION for guidance. Generate exactly 5 diverse, realistic meal recommendations for the specified meal type (${mealType}), tailored to the user's nutritional targets, remaining nutrition, dietary preferences, and goal. Each meal should be balanced, include a variety of ingredients, and fit within the adjusted targets (±10%).
...
Output exactly 5 meals in the following JSON array format (array of objects). NO OTHER TEXT:
[
  {
    "name": "Meal Name 1",
    "calories": number,
    ...
    "fat": number,
    "ingredients": [{"name": "Ingredient", "weight": "150g"}],
    "preparation": "Description of how to prepare",
    "allergens": ["list", "of", "potential", "allergens"],
    "alignment": "How it fits user's targets",
    "insights": "Health insights and suggestions"
  },
  ... (4 more)
]`;
  ...
  private buildImageAnalysisPrompt(
    ...
    return `You are an expert food analyst and dietitian. Analyze the provided image to identify all visible food items, ingredients, and portions. Estimate the nutritional content (macros: calories, protein, carbs, fat) for the entire meal shown, including breakdowns for each component. Provide amounts/weights for all ingredients and total macros. Include probable preparation method, potential allergens, alignment with user's targets, and health insights.
...
📋 JSON FORMAT (single detailed meal with full breakdowns):
{
  "name": "Complete Meal Name",
  "calories": ${Math.round(thisMealTarget.calories)},
  "protein": ${Math.round(thisMealTarget.protein)},
  "carbs": ${Math.round(thisMealTarget.carbs)},
  "fat": ${Math.round(thisMealTarget.fat)},
  "ingredients": [{"name": "Ingredient 1", "weight": "150g", "calories": number, "protein": number, "carbs": number, "fat": number}],
  "preparation": "Description of how the meal might be prepared",
  "allergens": ["list", "of", "potential", "allergens"],
  "alignment": "How it fits user's targets (e.g., 'High in protein, adjust carbs down')",
  "insights": "Health insights and suggestions"
    ...
    try {
      ...
      if (jsonMatch) {
        ...
        if (Array.isArray(parsed)) {
          // For meal recommendations (array of 5)
          return parsed.map((item: any, index: number) => ({
            id: `ai_${mealType}_${Date.now() + index}`,
            name: item.name || `Recommended Meal ${index + 1}`,
            calories: Math.round(parseFloat(item.calories) || 0),
            protein: Math.round(parseFloat(item.protein) || 0),
            carbs: Math.round(parseFloat(item.carbs) || 0),
            fat: Math.round(parseFloat(item.fat) || 0),
            category: mealType,
            ingredients: item.ingredients || [],
            // Additional fields can be stored or extended in FoodItem type
          }));
        } else {
          // For image analysis (single object)
          return [{
            id: `ai_${mealType}_${Date.now()}`,
            name: parsed.name || 'Analyzed Meal',
            calories: Math.round(parseFloat(parsed.calories) || 0),
