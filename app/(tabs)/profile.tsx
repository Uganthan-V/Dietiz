...
//   const completedMealsCount = meals.filter((meal: any) => meal.hasFood && meal.isEaten).length;
//   const totalMealsWithFood = meals.filter((meal: any) => meal.hasFood).length;
//   const mealsProgress = totalMealsWithFood > 0 ? `${completedMealsCount}/${totalMealsWithFood}` : '0/0';
//   const mealsPercentage = totalMealsWithFood > 0 ? Math.round((completedMealsCount / totalMealsWithFood) * 100) : 0;
...
//             </View>
//             <View style={styles.userInfo}>
//               <Text style={styles.userName}>{personalInfo?.name || "you"}</Text>
//               {/* <Text style={styles.userEmail}>tanim@example.com</Text> */}
//             </View>
//           </View>
//         </View>
...
//               title="Meals Completed"
//               value={mealsProgress}
//               subtitle={`${mealsPercentage}% today`}
//             />
//           </View>
//         </View>
...
const ProfileScreen = () => {
  ...
  // ✅ FIXED: REAL MEAL STATUS CHECK
  const completedMealsCount = meals.filter((meal: any) => meal.hasFood && meal.consumed).length;
  const totalMealsWithFood = meals.filter((meal: any) => meal.hasFood).length;
  const mealsProgress = totalMealsWithFood > 0 ? `${completedMealsCount}/${totalMealsWithFood}` : '0/0';
  const mealsPercentage = totalMealsWithFood > 0 ? Math.round((completedMealsCount / totalMealsWithFood) * 100) : 0;

  // ✅ REAL CALORIE PROGRESS
  const caloriePercentage = nutritionalData.calories > 0 
    ? Math.round((totalNutrition.calories / nutritionalData.calories) * 100) 
    : 0;
      ...
      <ScrollView showsVerticalScrollIndicator={false}>
          ...
          >
            ...
            <View style={styles.userInfo}>
              <Text style={styles.userName}>
                {personalInfo?.name || "Complete your profile"}
              </Text>
              {personalInfo?.age && (
                <Text style={styles.userDetails}>
                  {personalInfo.age} years • {personalInfo.gender || 'Not set'}
                </Text>
              )}
            </View>
        ...
        <View style={styles.statsSection}>
          ...
          <View style={styles.statsGrid}>
            <ProfileCard
              ...
              value={`${totalNutrition.calories} kcal`}
              subtitle={`${caloriePercentage}% of ${nutritionalData.calories} kcal goal`}
            />
            <ProfileCard
              icon="checkmark-circle"
              color="#4CAF50"
              title="Meals Completed"
              value={mealsProgress}
              subtitle={`${mealsPercentage}% completed today`}
            />
            <ProfileCard
              icon="trending-up"
              color="#2196F3"
              ...
              value="3 days"
              subtitle="Meals logged consecutively"
            />
          </View>
        </View>
        ...
        <View style={styles.achievementsSection}>
          ...
          <AchievementCard
            ...
            title="Protein Master"
            description="Meet protein goals for 5 consecutive days"
            achieved={completedMealsCount >= 3}
          />
          <AchievementCard
            icon="leaf"
            title="Healthy Eater"
            description="Stay within calorie goals for 10 days"
            achieved={caloriePercentage <= 110 && caloriePercentage >= 90}
          />
          <AchievementCard
            icon="fitness"
            title="Consistency King"
  ...
  onComplete={() => setShowPersonalInfoModal(false)}  // ✅ THIS FIXES IT
/>
    </SafeAreaView>
  );
};
